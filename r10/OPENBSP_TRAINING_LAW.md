...
| **Routing (training)** | `r10/routing/data/*` + `resolver.py` + `routing-eval.py` — inject in `training-chat-server.py` only |
...
**Routing law:** prod branch snapshot + systemic resolver + per-turn `[R10_ROUTING_CONTEXT]` in local training-chat (mimics prod lead state). Not in `instructions` markdown, not in Edge.
